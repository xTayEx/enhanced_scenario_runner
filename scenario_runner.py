...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        if LooseVersion(dist.version) < LooseVersion("0.9.15"):
            raise ImportError(
                f"CARLA version 0.9.15 or newer required. CARLA version found: {dist}"
            )
    ...
    def _get_scenario_class_or_fail(self, scenario):
        ...
        for scenario_file in scenarios_list:
            ...
            local_classes = [
                ...
                if c.__module__ == scenario_module.__name__
            ]
    ...
    def _prepare_ego_vehicles(self, ego_vehicles):
        ...
        if not self._args.waitForEgo:
            ...
            for i, _ in enumerate(self.ego_vehicles):
                ...
                CarlaDataProvider.register_actor(
                    self.ego_vehicles[i], ego_vehicles[i].transform
                )
    ...
    def _load_and_run_scenario(self, config, hooks: Optional[List[HookBase]] = None):
        """
    ...
    def _run_scenarios(self, hook_groups: Optional[List[List[HookBase]]] = None):
        """
        ...
        hook_groups_iter = iter(hook_groups)

        # Execute each configuration
        for config in scenario_configurations:
            hooks = next(hook_groups_iter, [])
            for _ in range(self._args.repetitions):
                self.finished = False
                result = self._load_and_run_scenario(config, hooks)
    ...
    def run(self, hook_groups: Optional[List[List[HookBase]]] = None):
        """
        ...
        if self._args.openscenario:
            ...
            result = self._run_scenarios(hook_groups)
